/**
 * setup-db-local.mjs
 * Run locally: node setup-db-local.mjs
 * Resets & Seeds local PGlite database with 20 sample records across all modules.
 */
...
async function main() {
  console.log("📦 Initializing local PGlite database at ./local-db-data...\n");

  const dbPath = path.resolve(__dirname, "local-db-data");
  const db = new PGlite(dbPath);

  // 1. Auth schema setup
  console.log("⚙️  Creating auth schema and auth.users table...");
  const authSetup = `
  ...
  // 2. Schema application statement by statement
  ...
  for (const stmt of statements) {
    try {
      ...
      if (err.message.includes("already exists") || err.message.includes("duplicate")) {
        ...
        console.warn(`  ⚠️  Warning executing statement: ${stmt.substring(0, 60)}... -> ${err.message}`);
        skipped++;
      }
    }
  }
  console.log(`✅ Schema processing finished: ${applied} statements executed, ${skipped} skipped.`);

  // 3. Clear existing sample data for fresh 20 samples rebuild
  console.log("🧹 Cleaning old sample records...");
  try {
    await db.exec(`
  ...
  // 6. Room Types
  console.log("🛏️  Seeding 6 Room Types...");
  const roomTypes = [
  ...
  console.log("\n─────────────────────────────────────");
  console.log("  ✅ 20 Sample Datasets Successfully Created!");
  console.log("─────────────────────────────────────");
  console.log("  • 20 Rooms (Rooms 101 - 502)");
  console.log("  • 20 Guests & Reservations (RES-10001 - RES-10020)");
  console.log("  • 20 Maintenance Tickets (MNT-1001 - MNT-1020)");
  console.log("  • 20 Hotel Inventory Items");
  console.log("─────────────────────────────────────\n");
}
